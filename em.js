...
function norm(x, u, s) {
      // Calculate Probability at point x 
      // given mean (u) and stdev (s)

      var y = (x - u)/s;
      //var lp = (-y*y)/2. - 0.5*Math.log(2 * Math.PI) - Math.log(s);
      var p = Math.exp(-(y*y)/2.) / Math.sqrt(2 * Math.PI)
      return(p/s);
}

function printModel(i, model) {
      var modelStr = i + ": ";
      for(var k = 0; k < model.length; k++) {
            modelStr = modelStr + "[" + Math.round(model[k][0]*100)/100 + ", " + 
                       Math.round(model[k][1]*100)/100 + ", " + Math.round(model[k][2]*100)/100 + "] "
      }
...
function weight(x, z, model) {
      // Calculate the posterior that class is z
      var probs = []; 
      var probTot = 0;
      for(var k = 0; k < model.length; k++) {
            var p = norm(x, model[k][0], Math.sqrt(model[k][1])) * model[k][2]; // prob point x is from k
            probs.push(p);
            probTot += p;
      }

      return(probs[z]/probTot);
}

function logNorm(x, u, s) {
      // log normal probability of x given u,s
      return(-0.5 * Math.log(2 * Math.PI * s * s) + ((x - u)*(x - u)/ (2*s)));
}

function logLikelihood(data, model) {
      var ll = ln = 0;
      var dp = null;
      ...
      for(var i = 0; i < data.length; i++) {
            ...
            ln = 0;
            for(var k = 0; k < model.length; k++) {
                  ln += model[k][2] * norm(dp, model[k][0], Math.sqrt(model[k][1]));
            }
            ll += Math.log(ln);
      }
      //console.log(ll);
...
function EM(data, plotFn, nGauss = 2, randomness = 0.1, epsilon = 1) {
      var curIter = 0;
      var modStr = '';
      var lastLL = curLL = null;
      var stationary = false;
      var model = [];
      //var nz = NormalZigg();

      var range = (Math.max(...data) - Math.min(...data))/nGauss;
      for(var k = 0; k < nGauss; k++) {
            //model.push([choice(data), 0.1, 1/nGauss]); // mean, variance, mix/phi (from multinomial)
            model.push([Math.random() * range + k*range + Math.min(...data), Math.sqrt(range), 1/nGauss]);

      }
      
      var iterEM = function() {
            if(!stationary) {
                  // Calculate expectations/weights
                  var w = [];
                  for(var k = 0; k < nGauss; k++) {
                        // Prob for Gaussian k and data point j
                        w.push([]);
                        for(var j = 0; j < data.length; j++) {
                              w[k].push(weight(data[j], k, model));
                        }
                  }
                  ...
                  for(var k = 0; k < nGauss; k++) {
                        ...
                        var tot = 0;
                        for(var j = 0; j < data.length; j++) {
                              s += w[k][j] * Math.pow(data[j] - model[k][0], 2); // Weigthed by posterior point j is z
                              tot += w[k][j];
                        }
                        ...
                        // Calculate mixing (phi)
                        var m = 0;
                        for(var j = 0; j < data.length; j++) {
                              m += w[k][j];
                        }
                        model[k][2] = m/data.length;
                  }
                  
                  // Normalize mixing (multinomial should sum to 1)
                  var mTot = 0;
                  for(var k = 0; k < nGauss; k++) {
                        mTot += model[k][2];
                  }
                  for(var k = 0; k < nGauss; k++) {
                        model[k][2] = model[k][2]/mTot;
                  }

                  modStr = printModel(curIter, model);

                  // Log likelihood of current model
                  curLL = logLikelihood(data, model);
                  plotFn(model, w, curLL, curIter);
                  curIter++;

                  if(lastLL !== null && curLL - lastLL < epsilon) {
                        stationary = true;
                  }
                  lastLL = curLL;
            }
            return({'modstr': modStr, 'stationary': stationary});
      }
